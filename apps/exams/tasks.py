...
def calculate_session_score(session_id):
    ...
    try:
            ...
            if answer.selected_option:
                if answer.selected_option.is_correct:
                    ...
                    if (
                        ...
                        and answer.question.allow_negative_marking
                    ):
                        negative = answer.question.negative_marks
        ...
        logger.info(
            f"Score calculated for session {session_id}: {percentage_score:.2f}%"
        )
        return {"success": True, "score": percentage_score, "passed": is_passed}
...
def update_exam_statistics(exam_id):
    ...
    try:
        ...
        if results.exists():
            ...
            logger.info(f"Updated statistics for exam {exam_id}")
...
def generate_exam_analytics(exam_id):
    ...
    try:
        ...
        analytics = {
            ...
            "average_score": results.aggregate(avg=Avg("score_percentage"))["avg"],
            "std_deviation": results.aggregate(std=StdDev("score_percentage"))["std"],
            "pass_rate": (
                results.filter(is_passed=True).count() / results.count() * 100
            ),
            "average_time": results.aggregate(avg=Avg("time_taken_seconds"))["avg"],
            "score_distribution": {
                "0-20": results.filter(score_percentage__lt=20).count(),
                "20-40": results.filter(
                    score_percentage__gte=20, score_percentage__lt=40
                ).count(),
                "40-60": results.filter(
                    score_percentage__gte=40, score_percentage__lt=60
                ).count(),
                "60-80": results.filter(
                    score_percentage__gte=60, score_percentage__lt=80
                ).count(),
                "80-100": results.filter(score_percentage__gte=80).count(),
            },
        }
